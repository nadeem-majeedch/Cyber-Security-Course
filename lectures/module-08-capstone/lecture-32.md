...
- **Data Science track:** a strong defense shows the anomaly pipeline's *measured* precision/recall at its chosen operating point (from the L21/L24 labs), presents the drift-monitoring plan as a detection-engineering deliverable rather than a notebook detail, and answers the panel's "what does your model miss?" with the documented adversarial failure modes and their mitigations from L23 — dataset provenance and label quality are artifacts here, exactly as PCAPs are for CS-track teams.
